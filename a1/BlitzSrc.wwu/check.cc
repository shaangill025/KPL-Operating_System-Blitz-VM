...
void assignOffsetsInMethOrFunction (MethOrFunction * methOrFunction) {
  ...
  // Make sure this space is divisible by 4 --- Phil Nelson, 2013
  if (methOrFunction->maxArgBytes % 4 == 0) 
    lastOffset -= methOrFunction->maxArgBytes;
  else
    lastOffset -= methOrFunction->maxArgBytes + 4 - methOrFunction->maxArgBytes % 4;
