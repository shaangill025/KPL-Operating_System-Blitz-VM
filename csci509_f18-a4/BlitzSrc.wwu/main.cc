...
//
// Modification by:
//   03/28/12 - Phil Nelson -- .c -> .k for source.
...
const char * inputFileNames [MAX_INPUT_FILES+1];   // Array of ptrs to file names
...
*****/
  ...
  const char *retName = initScanner (headerFileName);
  if (retName != NULL) {
  ...
  while (hdr != NULL) {
    ...
    while (uses != NULL) {
      ...
      if (! headerMapping->alreadyDefined (newPackName)) {
        ...
        fileName [len+1] = 'h';
        fileName [len+2] = '\0';
	const char *retName = initScanner (fileName);
	free(fileName);
        if (retName != NULL) {
